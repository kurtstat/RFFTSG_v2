...
df_ed_day <-
  ...
  filter(arrival_datetime <= as.POSIXct('2015-03-07 23:59',
                                        tz = "UTC")) |> 
  filter(departure_datetime >= as.POSIXct('2015-03-07 00:00',
                                          tz = "UTC"))
...
df_1441_a <-
  df_1441 |> 
  left_join(df_ed_day,
            join_by(date_hour_minute >= arrival_datetime,
                    date_hour_minute < departure_datetime)) |> 
  group_by(date_hour_minute) |> 
  summarize(ed_fullness = n())


# Step 3a - calculate the minimum and maximum fullness --------------------
...
# Step 6 - try Michael Duvekot's geom_area() solution ---------------------

ggplot(data = df_1441_a) +
  aes(x = date_hour_minute,
      y = ed_fullness) +
  geom_area(colour = colorspace::darken("#e76022", 10),
            fill = grid::linearGradient(
              x1 = 0, 
              x2 = 0, 
              y1 = 0, 
              y2 = 40,
              colours = c("#ffffff", "#e76022"))) +
  #geom_line(colour = "#e6550d") +
  geom_segment(x = as.numeric(min_fullness),
               y = 0,
               xend = as.numeric(min_fullness),
